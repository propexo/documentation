## Introduction
Propexo offers a unified API (AccessAPI) that simplifies integrating customer applications with property management software. This API uses two approaches to synchronization: Back-Sync and Date-Limited Sync. 
...
## Propexo AccessAPI Overview
AccessAPI is an intermediate layer between customer applications and Property Management Systems (PMS). It delivers cached data hydrated by an Extract, Transform, Load (ETL) process. This ETL implements one of the following syncing strategies:
