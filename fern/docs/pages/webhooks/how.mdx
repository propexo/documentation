...
Optionally, many customers use webhooks as a notification system and then query the associated AccessAPI endpoint to retrieve a full data record. For example, you may subscribe to the `DATA_UPDATE` event for the `leases` model. When you receive a webhook event, you can then query the `/leases/:id` endpoint to retrieve the full lease record.
