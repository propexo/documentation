...
Propexo makes sure to validate the input data for write requests as much as possible at AccessAPI so you can be informed of any possible errors as soon as possible.
...
To see this error, you can use our [write operation status endpoint](https://docs.propexo.com/api-reference/api-reference/admin/check-the-status-of-a-write-operation) (`GET /v1/admin/write-status/:root_job_id`), where `root_job_id` is the `job_id` field returned from a successful write operation request to AccessAPI:
